...
|---|---|---|
| Aurora | Teal, violet and green curtains | 8 beats |
...
For Pioneer/CDJ tempo, enable the Pro DJ Link plugin's tempo forwarding switch ("Sync to audio"). WasmLED displays the external player when that lease is active. Despite the switch name, WasmLED only uses BPM, not audio levels or beat triggers. Its clock uses actual host elapsed time, so slower plugin tick rates do not slow the pattern. Aurora now takes 8 beats per primary cycle (3.7 seconds at 130 BPM); set Speed to 2× or 4× for faster movement.
