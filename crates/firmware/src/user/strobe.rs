...
//         logo,
//         midi::{
//             DDJ_LEFT_BEAT_JMP_0_0, DDJ_LEFT_BEAT_JMP_0_1, DDJ_LEFT_BEAT_JMP_1_0,
//             DDJ_LEFT_BEAT_JMP_1_1, DDJ_LEFT_BEAT_JMP_2_1, DDJ_LEFT_BEAT_SYNC, DDJ_LEFT_CUE,
//             DDJ_LEFT_CUE_ROUND, DDJ_LEFT_RELOOP, MOVING_HEAD_TILT_ANIM_SPEED, STROBE_ALTERNATING,
...
//         },
//         state::{LogoMode, StrobeAnimation, StrobeAnimationAlternatingState, StrobeControls},
//         video,
//     },
// };
...
use super::{beat::DropState, config, state::State};

pub fn tick_on_beat(dmx: &mut [u8], input: TickInput, state: &mut State) {
    let master_brightness = 255;

    match state.drop_filter.state {
        DropState::None => unreachable!(),
        DropState::Begin => {
            println!("drop begin: {}", input.time);
            for g in state.config.strobe_groups.iter_mut() {
                if !g.supports_burst {
                    continue;
                }

                g.set_alpha(master_brightness, dmx);
                g.set_burst(true, dmx);
            }
        }
        DropState::Main => {
            println!("drop main: {}", input.time);
            for g in state.config.strobe_groups.iter_mut() {
                g.state.strobe_is_white_state = true;
                g.set_alpha(master_brightness, dmx);
            }
        }
    }
    ...
    //         // TODO: burst strobe?
    //         for group in state.config.strobe_groups.iter_mut() {
    //             if !group.supports_burst || !group.group.enabled {
    //                 group.blackout(dmx);
    //                 continue;
    //             }

    //             group.set_color(Color::white(), dmx);
    //             group.set_alpha(
    //                 state.animation.strobe.controls.brightness
    //                     * state.animation.strobe.white_value as u8,
    //                 dmx,
    //             );
    //         }
    //     }
    ...
    //                 }
    //                 group.set_color(Color::white(), dmx);
    //                 group.set_alpha(state.animation.strobe.controls.brightness, dmx);
    //             }
    ...
    match state.drop_filter.state {
        ...
        DropState::Main => {
            for g in state.config.strobe_groups.iter_mut() {
                ...
                if g.state.strobe_is_white_state {
                    g.set_alpha(0, dmx);
                    g.state.strobe_is_white_state = false;
                }
            }
        }
    }
...
// pub fn blackout(state: &mut State, dmx: &mut [u8]) {
//     for group in state.config.strobe_groups.iter_mut() {
//         group.blackout(dmx);
//     }
// }
...
//                 for group in state.config.strobe_groups.iter_mut() {
//                     if !group.supports_burst {
//                         continue;
//                     }

//                     group.set_alpha(0, dmx);
//                 }
//             } else {
...
//     // TODO: Make this configurable per-moving-head.
