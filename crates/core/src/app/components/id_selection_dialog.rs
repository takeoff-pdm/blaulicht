...
// Like `selection_dialog`, but options are keyed: the key is returned instead of the
// displayed value. Options are paginated so long lists never overflow the dialog.
...
where
    ...
    K: Clone,
{
    let page_memory_id = egui::Id::new(("components::id_selection_dialog_page", label.clone()));
    ...
    let options_vec: Vec<(K, T)> = options.into_iter().collect();
    ...
    let dialog_response = Dialog::new(label, dialog_dim)
        ...
        .show(ctx, |ui| {
            ...
            let page_items: Vec<&(K, T)> = pagination
            ...
            pagination.ui(ui, |ui| {
                ...
                for (idx, (option_key, option)) in page_items.iter().enumerate() {
                    let is_active = current_selection == *option_key;

                    if super::button(ui, is_active, &option.to_string(), button_size) {
                        selection = option_key.clone();
                        changed = true;
                        *is_open = false;
