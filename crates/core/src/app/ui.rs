                                ...
                                egui::ScrollArea::both()
                                    ...
                                    .show(ui, |ui| {
                                        if let Some(ref ops) = ops_copy {
                                            let mut idx = 0usize;
                                            render_plugin_ops(ui, ops, &mut idx, &data_clone);
                                        } else {
            ...
            Op::BeginTabs { id } => {
                ...
                *idx += 1;
                ...
                if let Some((_, _, s, e)) = tabs.iter().find(|(tid, _, _, _)| tid == current) {
                    let mut inner_idx = *s;
                    while inner_idx < *e {
                        render_plugin_ops(ui, ops, &mut inner_idx, data);
                    }
                ...
                *idx += 1;
                ...
                if resp.clicked() {
                    if let Some(pos) = resp.interact_pointer_pos() {
                        ...
                        let evt = ControlEvent::PluginUi(PluginUiEvent::CanvasClick {
                            id: *id,
                ...
                if resp.dragged() {
                    if let Some(pos) = resp.interact_pointer_pos() {
                        ...
                        let evt = ControlEvent::PluginUi(PluginUiEvent::CanvasDrag {
                            id: *id,
                            x: (local_pos.x * scale_x) as i32,
                            y: (local_pos.y * scale_y) as i32,
                ...
                if let Some(zoom_delta) = multi_touch {
                    ...
                    if zoom != 1.0 {
                        if let Some(pos) = resp.hover_pos() {
                            let local_pos = pos - rect.min;
                            let evt = ControlEvent::PluginUi(PluginUiEvent::CanvasPinch {
                                id: *id,
                                x: (local_pos.x * scale_x) as i32,
                                y: (local_pos.y * scale_y) as i32,
                                delta: zoom - 1.0,
